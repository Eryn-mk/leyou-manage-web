        ...
        <v-card>
                  ...
                  <v-card v-for="(spec,index) in specifications" :key="index" class="my-3">
                    <v-card-title>
                      <strong><input type="text" style="font-size: 16px" v-model="spec.group"
                                     placeholder="Please provide a category name"></strong>
                      <v-spacer/>
                      ...
                      <v-expansion-panel inset>
                        <v-expansion-panel-content v-for="(param,i) in spec.params" :key="i">
                          <v-layout wrap row slot="header">
                            <input placeholder="Please provide a attribute" size="15" v-model="param.k" @click.stop=""/>
                            <v-spacer/>
                          ...
                          <v-card class="mx-5">
                            ...
                            <v-card-text v-if="param.options.length > 0">
                              <v-layout row v-for="j in param.options.length" :key="j">
                                <input placeholder="Please provide a parameter" v-model="param.options[j-1]"/>
                                <v-spacer/>
                    ...
                    <v-card-actions>
                      <v-flex offset-xs8>
                        <v-btn small @click="addParam(index)">Add a new parameter</v-btn>
                      </v-flex>
                    </v-card-actions>
                  </v-card>
                </v-flex>
          ...
          <v-card-actions class="elevation-10">
            <v-flex offset-xs4 xs4>
              <v-btn block center color="primary" @click="saveTemplate">Save this template</v-btn>
            </v-flex>
...
//模板删除
        if (this.specifications.length === 0){
          //console.log("Delete:"+this.currentNode.id);
