  ...
  export default {
    ...
    methods: {
      submit() {
        ...
        console.log("Product description："+this.goods.spuDetail.description);
    ...
    watch: {
      ...
      "goods.categories": {
        ...
        handler(val) {
          // 判断商品分类是否存在，存在才查询
          if (val && val.length > 0) {
            const my_cid = this.goods.categories[2].id;
            // 根据分类查询品牌
            this.$http.get("/item/brand/cid/" + my_cid)
              .then(({ data }) => {
                this.brandOptions = data;
              });
            // 根据分类查询规格参数
            this.$http.get("/item/spec/params?cid=" + my_cid)
              .then(({ data }) => {
                let specs = [];
                let template = [];
                if (this.isEdit){
